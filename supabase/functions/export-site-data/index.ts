...
async function buildDataJs(): Promise<{ content: string; counts: Record<string, number>; skipped: string[]; withheld: string[] }> {
  ...
  /* Long-form text is pulled out into a second file. It was 80% of js/data.js —
     every article body and product article, in four languages — and the home
     page, the catalogue and the pairing guide download it without ever
     rendering a word of it: 444 KB gzipped instead of 39 KB, on the critical
     path, parser-blocking. js/data-loader.js only requests this file on the two
     pages that render long-form text, and js/main.js folds it back onto the
     same objects, so nothing downstream changes shape. */
  const articlesOut = {
    insights: Object.fromEntries(
      insights.filter((a: any) => a.body).map((a: any) => [a.slug, a.body]),
    ),
    products: Object.fromEntries(
      products.filter((p: any) => p.article).map((p: any) => [p.sku, p.article]),
    ),
  };
  const insightsLite = insights.map(({ body: _body, ...rest }: any) => rest);
  const productsLite = products.map(({ article: _article, ...rest }: any) => rest);

  const dbOut = { categories, scenarios, products: productsLite, tests: STATIC.tests, reports, insights: insightsLite, faqs, config: STATIC.config };
  ...
  const articlesHeader = `/* ============================================================
   VIEMAG — Article bodies  (AUTO-GENERATED, companion to js/data.js)
   Loaded only by insight.html and product.html, via the data-articles="1"
   attribute on their js/data-loader.js tag. js/main.js merges it back onto
   window.DB, so consumers still read a.body and p.article as before.
   ============================================================ */\n`;
  return {
    content: header + `window.DB = ${JSON.stringify(dbOut, null, 2)};\n`,
    articlesContent: articlesHeader + `window.DB_ARTICLES = ${JSON.stringify(articlesOut)};\n`,
    counts: {
...
async function commitToGithub(content: string, filePath = GITHUB_FILE_PATH): Promise<{ committed: boolean; message: string }> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const body = {
      message: `Export ${filePath.split('/').pop()} from Supabase [skip ci]`,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    /* Articles first. The two files are committed separately (the GitHub
       contents API is one path per call), so a failure between them leaves the
       site briefly mismatched — and that order makes the mismatch harmless: a
       js/data.js that still lists the old articles alongside a newer bodies
       file renders fine, whereas the reverse would show a new article with no
       body. */
    const articlesPath = GITHUB_FILE_PATH.replace(/data\.js$/, 'data-articles.js');
    const articlesResult = await commitToGithub(built.articlesContent, articlesPath);
    const result = await commitToGithub(built.content);
    result.message += ` · ${articlesPath.split('/').pop()}: ${articlesResult.message}`;
