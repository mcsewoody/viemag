...
//   role = 'owner'           -> may invite / set_role / set_password / delete
// Changing your OWN password does not come through here at all: /admin calls
// supabase.auth.updateUser() with the user's own session.
//
...
/* Stricter than the project's Auth setting (6). Staff accounts can edit the
   whole public catalogue, so a password an owner types for a colleague should
   not be guessable; the colleague can change it to anything >= this after. */
...
function checkPassword(v: unknown): string {
  ...
  if (v.length < MIN_PASSWORD) throw new BadRequest(`password must be at least ${MIN_PASSWORD} characters`);
  if (v.length > MAX_PASSWORD) throw new BadRequest('password is too long');
  return v;
}
...
async function actionSetRole(caller: Caller, body: Record<string, unknown>) {
  requireOwner(caller);
  const role: Role = body.role === 'owner' ? 'owner' : body.role === 'editor' ? 'editor' : (() => {
    throw new BadRequest('role must be owner or editor');
  })();
  const target = await targetProfile(body.user_id);

  if (target.user_id === caller.id) throw new Forbidden('you cannot change your own role');
  if (target.role === 'owner' && role !== 'owner' && (await ownerCount()) <= 1) {
    throw new Forbidden('at least one owner must remain');
  }

  const { error } = await admin().from('admin_users').update({ role }).eq('user_id', target.user_id);
  if (error) throw new Error(`role update failed: ${error.message}`);
  return { user_id: target.user_id, role };
}

async function actionSetPassword(caller: Caller, body: Record<string, unknown>) {
  requireOwner(caller);
  const password = checkPassword(body.password);
  const target = await targetProfile(body.user_id);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'set_role':     result = await actionSetRole(caller, body); break;
      case 'set_password': result = await actionSetPassword(caller, body); break;
