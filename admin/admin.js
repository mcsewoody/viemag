...
(function () {
  ...
  function callExportFunction() {
    ...
    return sb.auth.getSession().then(function (sessionRes) {
      var token =
        (sessionRes.data.session && sessionRes.data.session.access_token) ||
        CFG.supabaseAnonKey;
      ...
      return fetch(CFG.supabaseUrl + "/functions/v1/translate-text", {
        ...
        .catch(function (err) {
          if (err && err.name === "AbortError")
            throw new Error(t("translateTimeout") || "Translation timed out");
          throw err;
        })
        .finally(function () {
      ...
      .then(function (result) {
        ...
        if (!result.httpOk) {
          statusEl.textContent =
            t("exportFailed") + (result.data.error || "HTTP error");
          return;
        }
  ...
  function optionLabel(table, fieldName, value) {
    var dict =
      OPTION_I18N[state.lang] &&
      OPTION_I18N[state.lang][table] &&
      OPTION_I18N[state.lang][table][fieldName];
  ...
  function fetchMyRole() {
    return sb
      ...
      .then(function (res) {
        state.myRole =
          (res.data && res.data.role) === "owner" ? "owner" : "editor";
        return state.myRole;
      });
  }
  ...
  document.getElementById("syncNowBtn").addEventListener("click", function () {
    ...
    callExportFunction()
      .then(function (result) {
        ...
        statusEl.textContent = result.data.committed
          ? t("exported")
          : t("exportNoChange");
      })
      .catch(function (err) {
  ...
  function renderSidebar() {
    ...
    nav.innerHTML =
      ...
      "<a" +
      (state.view.table === ACCOUNTS_VIEW ? ' class="active"' : "") +
      ' data-table="' +
      ACCOUNTS_VIEW +
      '">' +
      esc(t("accounts")) +
  ...
  */
  ...
  var PW_RULES = [
    ...
    {
      key: "contains_email",
      label: "pwRuleNotEmail",
      test: function (v, email) {
  ...
  function pwProblems(v, email) {
    return PW_RULES.filter(function (r) {
      return !r.test(v, email);
    }).map(function (r) {
      return r.key;
    });
  }
  ...
  function describePwError(msg) {
    ...
    return (
      ...
      m[1]
        .split(",")
        .map(function (key) {
          var rule = PW_RULES.filter(function (r) {
            return r.key === key;
          })[0];
  ...
  function renderAccountList(root) {
    ...
    callManageAdmins({ action: "list" })
      .then(function (data) {
        ...
        if (!isOwner)
          html +=
            '<p class="panel-note">' + esc(t("editorCannotManage")) + "</p>";
        html += '<div class="toolbar">';
        html +=
          '<input type="search" id="listSearch" placeholder="' +
          esc(t("search")) +
          '">';
        if (isOwner)
          html +=
        ...
        html +=
          "<th>" + esc(t("email")) + "</th><th>" + esc(t("colName")) + "</th>";
        html +=
          "<th>" +
          esc(t("colRole")) +
          "</th><th>" +
        ...
        users.forEach(function (u) {
          ...
          html +=
            "<td>" +
            esc(u.email) +
            (self ? '<span class="hint">' + esc(t("youTag")) + "</span>" : "") +
            "</td>";
          html += "<td>" + esc(u.display_name || "—") + "</td>";
          html +=
            '<td><span class="badge-status">' +
            esc(u.role === "owner" ? t("roleOwner") : t("roleEditor")) +
            "</span></td>";
          html += "<td>" + esc(fmtDate(u.last_sign_in_at)) + "</td>";
          html += '<td class="row-actions">';
          if (isOwner) {
            html +=
              '<button class="btn edit-btn">' + esc(t("edit")) + "</button>";
        ...
        search.addEventListener("input", function () {
          ...
          Array.prototype.forEach.call(
            document.querySelectorAll("#listBody tr"),
            function (tr) {
              tr.style.display =
                tr.textContent.toLowerCase().indexOf(q) === -1 ? "none" : "";
            },
          );
        });
          ...
          function (tr) {
            ...
            if (delBtn)
              delBtn.addEventListener("click", function () {
                if (
                  !window.confirm(
                    tf("confirmDeleteAccount", { email: u.email }),
                  )
                )
                  return;
                delBtn.disabled = true;
                status.className = "save-status";
                callManageAdmins({ action: "delete", user_id: uid })
                  .then(function () {
                    renderContent();
                  })
                  .catch(function (err) {
  ...
  function renderAccountForm(root) {
    ...
    var u = isNew
      ? null
      : (state.accountRoster || []).filter(function (x) {
          return x.user_id === state.view.id;
        })[0];
    if (!isNew && !u) {
      // roster cache lost (e.g. language switch) — go back and refetch
    ...
    html +=
      '<form class="form-card" id="accForm" autocomplete="off"><div class="form-grid">';

    html +=
      '<div class="field wide"><label for="accEmail">' +
      esc(t("email")) +
      "</label>";
    if (isNew) {
      ...
      html +=
        '<input id="accEmail" type="email" value="' +
        esc(u.email) +
        '" disabled>';
    ...
    html +=
      '<div class="field"><label for="accName">' +
      esc(t("displayName")) +
      ' <span class="hint">(' +
      esc(t("optional")) +
      ")</span></label>";
    html +=
      '<input id="accName" type="text" value="' +
      esc((u && u.display_name) || "") +
      '"></div>';

    html +=
      '<div class="field"><label for="accRole">' +
      esc(t("colRole")) +
      "</label>";
    ...
    html += "</select>";
    if (isSelf)
      html += '<span class="hint">' + esc(t("cannotEditOwnRole")) + "</span>";
    html += "</div>";

    html +=
      '<div class="field wide"><label for="accPw">' +
      esc(isNew ? t("initialPassword") : t("newPassword")) +
      (isNew
        ? ""
        : ' <span class="hint">(' + esc(t("leaveBlankToKeep")) + ")</span>") +
      "</label>";
    ...
    html +=
      '<input id="accPw" type="text" autocomplete="off"' +
      (isNew ? " required" : "") +
      ">";
    ...
    html +=
      ...
      esc(isNew ? t("create") : t("save")) +
      "</button>";
    html +=
      '<button type="button" class="btn" id="accCancelBtn">' +
      esc(t("cancel")) +
      "</button>";
    if (!isNew && !isSelf)
      html +=
        '<button type="button" class="btn btn-danger" id="accDelBtn">' +
        esc(t("delete")) +
        "</button>";
    ...
    function refreshRules() {
      ...
      Array.prototype.forEach.call(
        document.querySelectorAll("#pwRules li"),
        function (li) {
          li.className = neutral
            ? ""
            : bad.indexOf(li.dataset.rule) === -1
              ? "ok"
              : "bad";
        },
      );
    ...
    if (delBtn)
      delBtn.addEventListener("click", function () {
        if (!window.confirm(tf("confirmDeleteAccount", { email: u.email })))
          return;
        delBtn.disabled = true;
        callManageAdmins({ action: "delete", user_id: u.user_id })
          .then(function () {
            state.view = { table: ACCOUNTS_VIEW, mode: "list", id: null };
            renderContent();
          })
    ...
    document.getElementById("accForm").addEventListener("submit", function (e) {
      ...
      if (isNew || pw !== "") {
        ...
        if (bad.length) {
          status.className = "save-status error";
          status.textContent = describePwError(
            "weak_password:" + bad.join(","),
          );
          return;
        }
      }
      ...
      var payload = isNew
        ? {
            ...
            password: pw,
            display_name:
              document.getElementById("accName").value.trim() || null,
      ...
      callManageAdmins(payload)
        ...
        .catch(function (err) {
          ...
          status.className = "save-status error";
          status.textContent =
            t("accountActionFailed") + describePwError(err.message);
        });
    });
  }
  /* ---------------- delete confirmation ----------------
  ...
  function confirmWithPassword(opts) {
    return new Promise(function (resolve) {
      ...
      h +=
        '<button type="button" class="btn" id="pwCancel">' +
        esc(t("cancel")) +
        "</button>";
      h +=
        '<button type="button" class="btn btn-danger" id="pwOk">' +
        esc(t("confirmDeleteBtn")) +
        "</button>";
      h += "</div></div>";
      wrap.innerHTML = h;
      ...
      function submit() {
        var pw = input.value;
        if (!pw) {
          fail(t("passwordRequired"));
          return;
        }
        okBtn.disabled = true;
        errEl.hidden = true;
        sb.auth
          .signInWithPassword({ email: state.session.user.email, password: pw })
          .then(function (res) {
            if (res.error) {
              ...
              fail(
                /invalid login credentials/i.test(m) ? t("passwordWrong") : m,
              );
              return;
            }
            close(true);
          ...
          .catch(function (e) {
            fail((e && e.message) || t("passwordWrong"));
          });
      }
      okBtn.addEventListener("click", submit);
      wrap.querySelector("#pwCancel").addEventListener("click", function () {
        close(false);
      });
  ...
  function renderList(root) {
    ...
    var extraPromise = Promise.all(
      extras.map(function (x) {
        return sb
          .from(x.table)
          .select([x.key].concat(x.cols).join(","))
          .then(function (r) {
            var byKey = {};
            if (!r.error)
    ...
    Promise.all([
      ...
      if (res.error) {
        root.innerHTML =
          '<p class="save-status error">' + esc(res.error.message) + "</p>";
        return;
      }
      var rows = res.data || [];
      ...
      var statusField = def.fields.find(function (f) {
      ...
      if (def.note)
        html += '<p class="panel-note">' + esc(t(def.note)) + "</p>";
      html += '<div class="toolbar">';
      html +=
        '<input type="search" id="listSearch" placeholder="' +
        esc(t("search")) +
      ...
      if (statusField && def.statusFilter) {
        ...
        statusField.options.forEach(function (o) {
          ...
          html +=
            '<button class="chip" data-status="' +
            esc(o) +
            '">' +
            esc(optionLabel(tableName, statusField.name, o)) +
            " " +
            (counts[o] || 0) +
            "</button>";
        });
      ...
      if (!rows.length) {
        ...
        rows.forEach(function (r) {
          ...
          if (def.thumb || def.thumbFallback) {
            ...
            if (img) {
              html +=
                '<img class="row-thumb" src="' +
                esc(img) +
                '" alt="" loading="lazy">';
            } else if (fb) {
              // No photo uploaded yet — show which front-end illustration this row uses.
              html +=
                '<span class="row-thumb row-thumb-ph" title="' +
                esc(t("noPhotoUsesArt")) +
                '">' +
                esc(fb) +
                "</span>";
            } else {
          ...
          if (statusField)
            html +=
              ...
              esc(
                r.status
                  ? optionLabel(tableName, statusField.name, r.status)
                  : "—",
              ) +
              "</span></td>";
          html += '<td class="row-actions">';
          ...
          if (tableName === "products")
            html +=
              '<button class="btn clone-btn">' + esc(t("clone")) + "</button>";
        ...
        if (!canDelete)
          html +=
            '<p class="panel-note">' + esc(t("editorCannotDelete")) + "</p>";
      }
      root.innerHTML = html;
      ...
      function applyFilters() {
        ...
        Array.prototype.forEach.call(
          ...
          function (tr) {
            ...
            var matchesStatus =
              !activeStatus || tr.dataset.status === activeStatus;
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll(".status-filter .chip"),
        function (btn) {
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll(".edit-btn"),
        function (btn) {
          btn.addEventListener("click", function () {
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll(".clone-btn"),
        function (btn) {
          btn.addEventListener("click", function () {
            var tr = btn.closest("tr");
            var sourceId = tr.dataset.id;
            var sourceLabel =
              tr.children[def.thumb || def.thumbFallback ? 1 : 0].textContent;
            cloneProduct(
              sourceId,
              sourceLabel,
              ...
              confirmWithPassword({ what: label, warn: warn }).then(
                function (ok) {
                  ...
                  sb.from(tableName)
                    ...
                    .then(function (res) {
                      if (res.error) {
                        alert(t("deleteFailed") + res.error.message);
                        return;
                      }
  ...
  function fetchRelationOptions(tableName, labelField) {
    ...
    if (state.relationCache[cacheKey])
      return Promise.resolve(state.relationCache[cacheKey]);
    return must(
      sb
        .from(tableName)
  ...
  function cloneJoinRows(sourceId, newId) {
    var joinFields = SCHEMA.products.fields.filter(function (f) {
      return f.type === "relation_many";
    });
    return Promise.all(
      joinFields.map(function (f) {
        return must(
          sb.from(f.joinTable).select(f.joinTargetKey).eq(f.joinKey, sourceId),
          "load " + f.joinTable,
        ).then(function (data) {
          var rows = (data || [])
            .map(function (r) {
              var o = {};
              o[f.joinKey] = newId;
              o[f.joinTargetKey] = r[f.joinTargetKey];
              return o;
            })
            .filter(function (r) {
              return r[f.joinTargetKey] !== newId;
            });
          ...
          return sb
            ...
            .then(function (res) {
              if (res.error)
                throw new Error(f.joinTable + " (clone): " + res.error.message);
            });
        });
      }),
    );
    ...
    return must(
      ...
      return sb
        ...
        .then(function (res) {
          if (res.error)
            throw new Error(
              "product_development (clone): " + res.error.message,
            );
        });
    });
  }
  ...
  function cloneProduct(sourceId, sourceLabel, statusEl) {
    ...
    return Promise.all([
      must(
        sb.from("products").select("*").eq("id", sourceId).single(),
        "load products",
      ),
      fetchMyRole(),
    ])
      .then(function (both) {
        ...
        return writeRowSkippingMissingColumns(
          "products",
          values,
          true,
          null,
          ...
          return cloneJoinRows(sourceId, newId)
            .then(function () {
  ...
  function renderForm(root) {
    ...
    var rowPromise = isNew
      ? Promise.resolve({})
      : must(
          sb.from(tableName).select("*").eq("id", id).single(),
          "load record",
        );

    var relFields = def.fields.filter(function (f) {
      return f.type === "relation" || f.type === "relation_many";
    });
    var relOptionsPromise = Promise.all(
      relFields.map(function (f) {
        return fetchRelationOptions(f.table, f.labelField);
      }),
    ).then(function (results) {
      var map = {};
      relFields.forEach(function (f, i) {
        map[f.name] = results[i];
      });
      return map;
    });

    var joinFields = def.fields.filter(function (f) {
      return f.type === "relation_many";
    });
    var joinValuesPromise =
      isNew || !joinFields.length
        ? Promise.resolve({})
        : Promise.all(
            joinFields.map(function (f) {
              return must(
                sb.from(f.joinTable).select(f.joinTargetKey).eq(f.joinKey, id),
                "load " + f.joinTable,
              ).then(function (data) {
                return (data || []).map(function (r) {
                  return r[f.joinTargetKey];
                });
              });
            }),
          ).then(function (results) {
            var map = {};
            joinFields.forEach(function (f, i) {
              map[f.name] = results[i];
            });
            return map;
          });
    ...
    var costPromise =
      !subTab || isNew
        ...
        : must(
            sb
              .from("product_sales_cost")
              .select("sales_cost_usd")
              .eq("product_id", id)
              .maybeSingle(),
            "load sales cost",
          ).then(function (r) {
    ...
    Promise.all([
      ...
      .catch(function (err) {
        ...
        root.innerHTML =
          '<div class="empty-state"><p class="save-status error">' +
          esc(t("loadFailed") + err.message) +
          "</p>" +
          '<button class="btn" id="backBtn">&larr; ' +
          esc(t("backToList")) +
          "</button></div>";
  ...
  function formThumbInner(url, fallback) {
    ...
    if (fallback)
      return (
        '<span class="form-thumb-ph" title="' +
        esc(t("noPhotoUsesArt")) +
        '">' +
        esc(fallback) +
        "</span>"
  ...
  function buildFormHtml(ctx) {
    ...
    var head =
      "<h2>" +
      esc(tTable(ctx.tableName)) +
      " — " +
      (ctx.isNew ? esc(t("addNew")) : esc(ctx.row[def.title] || "")) +
      "</h2>";
    ...
    var html =
      '<div class="form-head"><div class="form-head-main">' +
      head +
      '<button class="btn" id="backBtn">&larr; ' +
      esc(t("backToList")) +
      "</button></div>";
    if (!ctx.isNew && (def.thumb || def.thumbFallback)) {
      html +=
        '<div class="form-thumb" id="formHeadThumb">' +
        formThumbInner(
          def.thumb ? ctx.row[def.thumb] : null,
          def.thumbFallback ? ctx.row[def.thumbFallback] : null,
        ) +
        "</div>";
    }
    html += "</div>";
      ...
      def.tabs
        .map(function (tb, i) {
          return (
            ...
            esc(tb.key) +
            '">' +
            esc((dict.productTabs && dict.productTabs[tb.key]) || tb.key) +
            "</button>"
          );
        })
        .join("") +
  ...
  function langRowHtml(ctx, srcName, srcRow, names) {
    ...
    var fields = names
      .map(function (n) {
        return srcDef.fields.filter(function (x) {
          return x.name === n;
        })[0];
      })
      .filter(Boolean);
    ...
    var pendingColumn = fields.some(function (f) {
      return (
        f.requiresColumn &&
        !Object.prototype.hasOwnProperty.call(srcRow, f.name)
      );
    });
    if (pendingColumn) {
      html +=
        '<div class="computed-value muted">' +
        esc(t("columnPending")) +
        "</div>";
      html += "</div>";
      return html;
    ...
    html +=
      ...
      (prefix === "product_article" ? " article-lang-inputs" : "") +
      '">';
    fields.forEach(function (f) {
      ...
      if (canTranslate) {
        ...
        html +=
          '<button type="button" class="lang-translate-btn" data-target="' +
          esc(f.name) +
          '" data-lang="' +
    ...
    if (canTranslate)
      html +=
        '<span class="translate-status" data-translate-status="' +
        esc(prefix) +
        '"></span>';
    html += "</div>";
    return html;
  ...
  function fieldBlockHtml(ctx, srcName, srcRow, f) {
    ...
    var html =
      '<div class="field' +
      (wideTypes.indexOf(f.type) !== -1 ? " wide" : "") +
      '" data-field="' +
      f.name +
      '">';
    ...
    if (f.internal && !ctx.def.tabs)
      html +=
        ' <span class="internal-tag">' + esc(t("internalField")) + "</span>";
    if (f.type === "computed" || f.readOnly)
      html += ' <span class="calc-tag">' + esc(t("calculated")) + "</span>";
    html += "</label>";
  ...
  function renderFieldInput(
    ...
    if (f.readOnly) {
      return (
        '<input type="number" disabled data-readonly-name="' +
        f.name +
        '" value="' +
    ...
    switch (f.type) {
      ...
      case "computed":
        return (
          '<div class="computed-value muted" data-compute="' +
          esc(f.compute) +
          '">—</div>'
        );
      case "textarea":
      ...
      case "number":
        return (
          '<div class="num-wrap"><input type="number" step="any" data-name="' +
          f.name +
          '" value="' +
          (value == null ? "" : esc(value)) +
          '">' +
          (f.unit ? '<span class="num-unit">' + esc(f.unit) + "</span>" : "") +
          "</div>"
        );
      case "date":
        return (
          '<input type="date" data-name="' +
          f.name +
          '" value="' +
          esc(value || "") +
          '">'
      ...
      case "boolean":
        return (
          '<div class="field checkbox"><input type="checkbox" data-name="' +
          f.name +
          '" ' +
          (value ? "checked" : "") +
          "></div>"
          ...
          f.options
            .map(function (o) {
              return (
                ...
                (value === o ? " selected" : "") +
                ">" +
                esc(optionLabel(tableName, f.name, o)) +
                "</option>"
              );
            })
            .join("") +
        ...
        return (
          ...
          f.options
            .map(function (o) {
              ...
              return (
                '<label><input type="checkbox" data-name="' +
                f.name +
                '" data-multi-value="' +
                esc(o) +
                '"' +
      ...
      case "relation_many": {
        ...
        return (
          '<div class="relation-many-options">' +
          mopts
            .map(function (o) {
  ...
  function renderImageField(name, urls, multi) {
    ...
    html +=
      ...
      name +
      '" value="' +
      esc(multi ? JSON.stringify(urls) : urls[0] || "") +
      '">';
    ...
    html +=
      ...
      '<label class="file-pick image-pick">' +
      esc(
        multi
          ? t("uploadImage")
          : urls[0]
      ...
      (multi ? "multiple" : "") +
      "></label>" +
      '<button type="button" class="btn image-clear"' +
      (urls.length ? "" : " hidden") +
      ">" +
      esc(t("removeImage")) +
      "</button>" +
  ...
  function textareaHtml(f, value, rows) {
    ...
    return (
      ...
      esc(f.name) +
      '">' +
      '<div class="rich-toolbar" aria-label="' +
      esc(t("formatToolbar")) +
      '">' +
      '<button type="button" data-format="undo" title="' +
      esc(t("formatUndo")) +
      '">Undo</button>' +
      '<button type="button" data-format="redo" title="' +
      esc(t("formatRedo")) +
      '">Redo</button>' +
      '<select class="rich-block-format" title="' +
      esc(t("formatBlock")) +
      '">' +
      '<option value="p">' +
      esc(t("formatParagraph")) +
      "</option>" +
      '<option value="h2">' +
      esc(t("formatHeading")) +
      "</option>" +
      '<option value="h3">' +
      esc(t("formatSubheading")) +
      "</option>" +
      "</select>" +
      '<button type="button" data-format="bold" title="' +
      esc(t("formatBold")) +
      '"><b>B</b></button>' +
      '<button type="button" data-format="italic" title="' +
      esc(t("formatItalic")) +
      '"><i>I</i></button>' +
      '<button type="button" data-format="bullet" title="' +
      esc(t("formatBullet")) +
      '">• List</button>' +
      '<button type="button" data-format="number" title="' +
      esc(t("formatNumber")) +
      '">1. List</button>' +
      '<button type="button" data-format="align-left" title="' +
      esc(t("alignLeft")) +
      '">Left</button>' +
      '<button type="button" data-format="align-center" title="' +
      esc(t("alignCenter")) +
      '">Center</button>' +
      '<button type="button" data-format="align-right" title="' +
      esc(t("alignRight")) +
      '">Right</button>' +
      '<button type="button" data-format="insert-divider" title="' +
      esc(t("insertDivider")) +
      '">Line</button>' +
      '<button type="button" data-format="spec-row" title="' +
      esc(t("insertSpecRow")) +
      '">+ Row</button>' +
      '<input type="text" class="rich-link-url" placeholder="' +
      esc(t("linkUrl")) +
      '">' +
      '<button type="button" data-format="link" title="' +
      esc(t("insertLink")) +
      '">Link</button>' +
      '<select class="rich-image-layout" title="' +
      esc(t("imageLayout")) +
      '">' +
      '<option value="wide">' +
      esc(t("imageLayoutWide")) +
      "</option>" +
      '<option value="right">' +
      esc(t("imageLayoutTextLeft")) +
      "</option>" +
      '<option value="left">' +
      esc(t("imageLayoutTextRight")) +
      "</option>" +
      "</select>" +
      '<input type="text" class="rich-image-url" placeholder="' +
      esc(t("imageUrl")) +
      '">' +
      '<button type="button" data-format="image" title="' +
      esc(t("insertImage")) +
      '">Img</button>' +
      '<button type="button" data-format="replace-selected-image" title="' +
      esc(t("replaceSelectedImage")) +
      '">Replace</button>' +
      '<button type="button" data-format="remove-selected-media" title="' +
      esc(t("removeSelectedMedia")) +
      '">Remove</button>' +
      '<label class="rich-file-pick" title="' +
      esc(t("uploadArticleImage")) +
      '">' +
      esc(t("uploadArticleImage")) +
      '<input type="file" class="rich-article-image-file" accept="image/jpeg,image/png,image/webp"></label>' +
      '<span class="rich-upload-status"></span>' +
      '<input type="text" class="rich-youtube-url" placeholder="' +
      esc(t("youtubeUrl")) +
      '">' +
      '<button type="button" data-format="youtube" title="' +
      esc(t("insertYoutube")) +
      '">YT</button>' +
      "</div>" +
      '<div class="rich-editor-body">' +
      '<div class="rich-visual-editor" contenteditable="true" data-rich-visual spellcheck="true" aria-label="' +
      esc(t("articleEditor")) +
      '"></div>' +
      hiddenTextarea +
      "</div>" +
  ...
  function richPreviewHtml(src) {
    ...
    function youtubeEmbedUrl(url) {
      ...
      if (host === "youtube.com" || host === "m.youtube.com") {
        ...
        else if (/^\/(?:embed|shorts)\//.test(u.pathname))
    ...
    function flushTable() {
      ...
      out.push(
        '<div class="rich-table-wrap"><table class="rich-table"><tbody>' +
          table
            .map(function (cells) {
              return (
                "<tr>" +
    ...
    String(src || "")
      ...
      .forEach(function (raw) {
        ...
        if (tableMatch) {
          flushPara();
          flushList();
          var cells = tableMatch[1].split("|").map(function (x) {
            return x.trim();
          });
          if (
            cells.length >= 2 &&
        ...
        if (/^(?:---|\*\s*\*\s*\*)$/.test(line)) {
          flush();
          out.push('<hr class="rich-divider">');
          return;
        }
        var media =
          /^!\[([^\]]*)\]\(([^)]+)\)(?:\{(wide|left|right)\})?$/i.exec(line);
        if (media) {
          flush();
          ...
          if (type === "youtube") {
            ...
            if (youtube)
              out.push(
                '<figure class="rich-youtube"><iframe src="' +
                  youtube +
                  '" title="YouTube video" loading="lazy" allowfullscreen></iframe></figure>',
              );
            return;
          }
          if (type === "video") {
            ...
            if (video)
              out.push(
                '<figure class="rich-video"><video src="' +
                  video +
  ...
  function wireRichEditor(wrap) {
    ...
    var hasHtml =
      /<\/?(?:p|h2|h3|ul|ol|li|figure|img|iframe|strong|em|a|div|section|br|hr|table|thead|tbody|tr|th|td)\b/i;
    ...
    function safeUrl(url, media) {
      ...
      if (
        media &&
        (u.charAt(0) === "/" || /^(?:\.\.?\/)?(?:assets|image)\//i.test(u))
      )
        return u;
      if (!/^https?:\/\//i.test(u)) return "";
      try {
    ...
    function cleanArticleHtml(src) {
      ...
      function clean(node) {
        ...
        if (
          tag === "section" ||
          (tag === "div" && node.classList.contains("rich-split"))
        ) {
          var splitClass = node.classList.contains("image-left")
            ? "image-left"
            : "image-right";
          return (
            '<section class="rich-split ' +
            splitClass +
            '">' +
            children +
            "</section>"
        ...
        if (tag === "p" || tag === "div") {
          if (node.classList.contains("rich-copy"))
            return '<div class="rich-copy">' + children + "</div>";
          if (node.classList.contains("rich-table-wrap"))
            return '<div class="rich-table-wrap">' + children + "</div>";
          var align =
            node.style && node.style.textAlign ? node.style.textAlign : "";
          var cls = /^(center|right)$/i.test(align)
            ? ' class="align-' + align.toLowerCase() + '"'
            : "";
        ...
        if (tag === "table")
          return (
            '<div class="rich-table-wrap"><table class="rich-table">' +
            children +
            "</table></div>"
          );
        ...
        if (tag === "figure") {
          ...
          if (/\brich-youtube\b/.test(clsRaw)) {
            var yt = youtubeEmbedUrl(
              node.getAttribute("data-youtube") ||
                (node.querySelector("iframe") &&
                  node.querySelector("iframe").getAttribute("src")) ||
                "",
            );
            return yt
              ? '<figure class="rich-youtube" data-youtube="' +
                  esc(yt) +
                  '"><iframe src="' +
                  esc(yt) +
                  '" title="YouTube video" loading="lazy" allowfullscreen></iframe></figure>'
      ...
      return Array.prototype.map
        .call(doc.body.firstChild.childNodes, clean)
        .join("");
    }
    ...
    function mediaFigureHtml(kind, src) {
      ...
      return (
        '<figure class="rich-image"><img src="' +
        esc(src) +
        '" alt=""></figure>'
      );
    }
    function insertMedia(kind, url) {
      ...
      if (layout === "wide") {
        insertHtml(
          figure.replace('class="rich-image"', 'class="rich-image wide"') +
            "<p><br></p>",
        );
        return true;
      }
      ...
      var media = figure.replace(
        'class="rich-image"',
        'class="rich-image wide"',
      );
      var body =
        layout === "left"
    ...
    function specRowDefaults() {
      var name = textarea.getAttribute("data-name") || "";
      if (/technical_content_vi$/.test(name))
        return { name: "Kích thước", value: "64 x 64 x 82 mm" };
      if (/technical_content_id$/.test(name))
        return { name: "Dimensi", value: "64 x 64 x 82 mm" };
      if (/technical_content_zh$/.test(name))
        return { name: "尺寸", value: "64 x 64 x 82 mm" };
      if (/technical_content_en$/.test(name))
        return { name: "Dimensions", value: "64 x 64 x 82 mm" };
      return { name: t("specRowName"), value: t("specRowValue") };
    }
    /* Size and weight rows on the Specifications tab are a two-column table.
       schema.js tells staff they can type "| Dimension | 64 mm |", and that is
       true exactly once: richPreviewHtml parses pipe rows when a field still
       holds PLAIN text. From the first edit onwards the field holds HTML, and a
       typed pipe row is then just a paragraph that reads "| Dimension | 64 mm |"
       on the live page. So the row has to go in as a real <tr>.

       No wrapper div here on purpose — cleanArticleHtml adds rich-table-wrap
       around every <table> on the way out, and inserting our own would nest one
       wrap inside another on every pass. */
    function insertSpecRow() {
      ...
      /* Caret is already in the table: add an empty row under the current one and
         put the caret in its first cell, so holding the button types a spec sheet
         top to bottom without reaching for the mouse. */
      var next = row.cloneNode(true);
      Array.prototype.forEach.call(
        next.querySelectorAll("td, th"),
        function (cell) {
          cell.innerHTML = "<br>";
        },
      );
      row.parentNode.insertBefore(next, row.nextSibling);
      var first = next.querySelector("td, th");
      if (first && sel) {
    ...
    visual.innerHTML = hasHtml.test(textarea.value)
      ? cleanArticleHtml(textarea.value)
      : richPreviewHtml(textarea.value);
    ...
    textarea.addEventListener("rich-source-changed", function () {
      visual.innerHTML =
        (hasHtml.test(textarea.value)
          ? cleanArticleHtml(textarea.value)
          : richPreviewHtml(textarea.value)) || "<p><br></p>";
      selectMedia(null);
      savedRange = null;
      textarea.value = cleanArticleHtml(visual.innerHTML).trim();
      textarea.dispatchEvent(new Event("input", { bubbles: true }));
    ...
    Array.prototype.forEach.call(
      wrap.querySelectorAll("[data-format]"),
      function (btn) {
        ...
        btn.addEventListener("click", function () {
          ...
          if (kind === "insert-divider") {
            insertHtml('<hr class="rich-divider"><p><br></p>');
            return;
          }
          if (kind === "spec-row") {
            insertSpecRow();
          ...
          if (kind === "image") {
            ...
            if ((replaceSelectedImage(url) || insertImageUrl(url)) && input)
              input.value = "";
            return;
          }
          if (kind === "replace-selected-image") {
            ...
            if (!replaceUrl || !replaceSelectedImage(replaceUrl)) {
          ...
          if (kind === "youtube") {
            ...
            if (!insertMedia("youtube", youtubeUrl)) {
  ...
  function costFieldNames() {
    return SCHEMA.product_development.fields
      .filter(function (f) {
        return /_usd$/.test(f.name) && !f.readOnly;
      })
      .map(function (f) {
        return f.name;
      });
  ...
  function refreshComputed(ctx) {
    var inputs = costFieldNames().map(function (n) {
      return document.querySelector('[data-name="' + n + '"]');
    });
    var salesCost = inputs[0]
      ? inputs.reduce(function (sum, el) {
          return sum + (el && el.value !== "" ? Number(el.value) : 0);
        }, 0)
      : ctx.viewCost == null
        ? null
        : Number(ctx.viewCost);

    var generatedEl = document.querySelector(
      '[data-readonly-name="sales_cost_usd"]',
    );
    if (generatedEl)
      generatedEl.value = salesCost == null ? "" : salesCost.toFixed(2);
    ...
    if (floor != null) {
      var under = pct < floor;
      txt +=
        " — " + tf(under ? "marginBelowFloor" : "marginOk", { min: floor });
      cls += under ? " warn" : " ok";
    }
  ...
  function wireFormEvents(ctx, id, joinFields) {
    ...
    Array.prototype.forEach.call(
      document.querySelectorAll(".image-field"),
      function (wrap) {
        wireImageField(wrap, ctx.tableName);
      },
    );
    Array.prototype.forEach.call(
      document.querySelectorAll(".file-field"),
      function (wrap) {
  ...
  function runTranslateButton(btn) {
    ...
    var statusEl = row
      ? row.querySelector('[data-translate-status="' + prefix + '"]')
      : null;
    ...
    if (!text) {
      if (statusEl) {
        ...
        statusEl.textContent = t("translateEmpty");
      }
      return;
    }

    Array.prototype.forEach.call(rowBtns, function (b) {
      b.disabled = true;
    });
    if (statusEl) {
      statusEl.className = "translate-status";
      statusEl.textContent = t("translating");
    }

    callTranslateFunction(text, source)
      .then(function (result) {
        Array.prototype.forEach.call(rowBtns, function (b) {
          b.disabled = false;
        });
        if (!result.httpOk) {
          if (statusEl) {
            statusEl.className = "translate-status error";
            statusEl.textContent =
              t("translateFailed") + (result.data.error || "HTTP error");
          }
          return;
        }
      ...
      .catch(function (err) {
        ...
        if (statusEl) {
          statusEl.className = "translate-status error";
          statusEl.textContent = t("translateFailed") + err.message;
        }
      });
  }
  ...
  function wirePrivateFileField(wrap) {
    ...
    function setPaths(next) {
      ...
      listEl.innerHTML = next
        .map(function (p) {
          ...
          return (
            ...
            esc(label) +
            "</span>" +
            '<button type="button" class="btn file-open" data-path="' +
            esc(p) +
            '">' +
            esc(t("openFile")) +
            "</button>" +
            '<button type="button" class="btn btn-danger file-del" data-path="' +
            esc(p) +
            '">' +
            esc(t("removeFile")) +
            "</button></li>"
        ...
        .join("");
      Array.prototype.forEach.call(
        listEl.querySelectorAll(".file-open"),
        function (btn) {
          btn.addEventListener("click", function () {
            statusEl.textContent = "";
            ...
            sb.storage
              .from(CFG.privateBucket)
              .createSignedUrl(btn.dataset.path, 120)
              .then(function (res) {
                if (res.error) {
                  statusEl.textContent = t("signFailed") + res.error.message;
                  return;
                }
      ...
      Array.prototype.forEach.call(
        listEl.querySelectorAll(".file-del"),
        function (btn) {
          btn.addEventListener("click", function () {
  ...
  function wireImageField(wrap, tableName) {
    ...
    function setUrls(urls) {
      ...
      previewEl.innerHTML = urls
        .map(function (u) {
          ...
          return (
            ...
            '<button type="button" class="btn thumb-open" data-open-url="' +
            esc(u) +
            '">' +
            esc(t("openFile")) +
            "</button>" +
            '<button type="button" class="btn btn-danger thumb-remove" data-remove-url="' +
            esc(u) +
            '">' +
            esc(t("removeImage")) +
            "</button>" +
      ...
      Array.prototype.forEach.call(
        previewEl.querySelectorAll(".thumb-open"),
        function (btn) {
          btn.addEventListener("click", function () {
      ...
      Array.prototype.forEach.call(
        previewEl.querySelectorAll(".thumb-remove"),
        function (btn) {
          btn.addEventListener("click", function () {
            setUrls(
              currentUrls().filter(function (u) {
                return u !== btn.dataset.removeUrl;
              }),
            );
            state.formDirty = true;
          });
      ...
      if (headEl && headDef && wrap.dataset.imageField === headDef.thumb) {
        var fbInput = headDef.thumbFallback
          ? document.querySelector(
              '[data-name="' + headDef.thumbFallback + '"]',
            )
          : null;
        headEl.innerHTML = formThumbInner(
          urls[0] || null,
          fbInput ? fbInput.value : null,
        );
      }
    }
    setUrls(currentUrls());
    ...
    def.fields.forEach(function (f) {
      ...
      if (f.type === "multiselect") {
        var checked = document.querySelectorAll(
          'input[data-name="' + f.name + '"][data-multi-value]:checked',
        );
        var vals = Array.prototype.map.call(checked, function (c) {
          return c.dataset.multiValue;
        });
  ...
  function writeRowSkippingMissingColumns(
    tableName,
    values,
    isNew,
    id,
      ...
      function (res) {
        ...
        var joinOps = joinFields.map(function (f) {
          var checks = document.querySelectorAll(
            'input[data-name="' + f.name + '"][data-rel-id]:checked',
          );
          var selectedIds = Array.prototype.map
            .call(checks, function (c) {
              return c.dataset.relId;
            })
          ...
          return sb
            ...
            .then(function (delRes) {
              if (delRes.error)
                throw new Error(
                  f.joinTable + " (clear): " + delRes.error.message,
                );
              if (!selectedIds.length) return null;
              var rows = selectedIds.map(function (targetId) {
                var o = {};
                o[f.joinKey] = rowId;
                o[f.joinTargetKey] = targetId;
                return o;
              });
              return sb
                .from(f.joinTable)
                ...
                .then(function (insRes) {
                  if (insRes.error)
                    throw new Error(
                      f.joinTable + " (rewrite): " + insRes.error.message,
                    );
                });
            });
        });
